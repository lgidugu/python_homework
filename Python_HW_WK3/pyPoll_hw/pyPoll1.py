...
with open("election_data_1.csv") as pfile:
    csv_reader = csv.reader(pfile, delimiter = ',')

total_election_data_csv = os.path.join("election_data_1.csv")
...
with open(total_election_data_csv, "r")as csvfile:
    csvreader = csv.reader(csvfile, delimiter = ",")
    sort = sorted(list_o_candidates, key = operator.itemgetter(2))
    counter = collections.Counter(list_o_candidates)

    for (k, v) in counter.items():
        percent_votes = round((int(v)/total_votes_cnt)*100, 1)
        print(k, ":%s %%\n"% percent_votes,"(",v,")")
